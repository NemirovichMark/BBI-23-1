...
namespace Lab8
{
    ...
    class Task_6 : Task
    {
        ...
        private void findSl(string text1)
        {
            int nSl = 0;
    ...
    class Task_8 : Task
    {
        ...
        public override void ParseText()
        {
            ...
            do
            {
                ...
                if (_text.Length >= 50)
                    _text = _text.Substring(nKol+1, _text.Length - nKol-1);
                else
                    _text = "";
    ...
    class Task_9_10 : Task
    {
        ...
        private void setTwoChars(char ch)
        {
            ...
            {
                Nchars++;
               if ( _text.Contains(cp[i].TwoChars))
                {
                    cp[i].key = ch;
                    break;
        ...
        private void TestChars()
        {
            ...
            {
                char ch = (char)i;
                bool isUseChar = _text.Contains(ch);
                if (!isUseChar)
                {
                    setTwoChars(ch);
        ...
        private void EncodeText()
        {
            for (int i = 0; i < Ncp; i++)
            {
                if (cp[i].key > (char)10) 
                {
                    ...
                    _text = _text.Replace(cp[i].TwoChars, ss);
        ...
        public void DecodeText()
        {
            for (int i = 0; i < Ncp; i++)
            {
                ...
                {
                    string ss = cp[i].key.ToString();
                    _text = _text.Replace(ss,cp[i].TwoChars );
        ...
        public override void ParseText()
        {
            ...
            {
                string obr = _text.Substring(i, 2);
                if (! FoundCP(obr)) 
                {
                   
                    cp[Ncp].TwoChars = obr;
                    cp[Ncp].count=(_text.Length - _text.Replace(obr, "").Length) / obr.Length;
                    cp[Ncp].key = (char)10;
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            string inpStr = "";
            Console.Write("Задайте текст для дальнейшей обработки:");
