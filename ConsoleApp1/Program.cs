...
{
        private Results[] Sort(Results[] arr)
        {
            if (arr == null || arr.Length <= 1)
                return arr;

            Results[] temp = new Results[arr.Length];
            MergeSortHelper(arr, temp, 0, arr.Length - 1);
            return arr;
        }

        private static void MergeSortHelper(Results[] arr, Results[] temp, int left, int right)
        {
            if (left < right)
            {
        ...
        private static void Merge(Results[] arr, Results[] temp, int left, int mid, int right)
        {
            ...
            while (i <= mid && j <= right)
            {
                if ((temp[i].result1 +  temp[i].result2) >= (temp[j].result1 + temp[j].result2))
                {
                    arr[k] = temp[i];
                    i++;
                }
...
/*public abstract class Jump
{
    ...
    public Jump(string name, double[] mark, string discipline)
    {
        _discipline = null;
        _name = name;
        _mark = mark;
...
{
    public three_meters(string name, double[] mark, string discipline) : base(name, mark, discipline)
    {
    }
...
{
    public five_meters(string name, double[] mark, string discipline) : base(name, mark, discipline)
    {
    }
    public override void display()
    {
...
public class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        {
 new three_meters("первый", new double [] {1, 2,3,4,444 }, "three"),
 new three_meters("второй", new double [] { 1, 2, 3, 4, 55 }, "three"),
 new five_meters("третий", new double [] { 1, 2, 3, 4, 3 }, "five"),
 new five_meters ("четвёртый", new double[] { 1, 2, 3, 4, 50 },"five"),
 new five_meters ("пятый", new double[] { 1, 2, 3, 4, 5 },"five")
        };
        var z = new Program();
        z.Sort(table);
...
public abstract class Country
{
    ...
    public static void Sort(Country[] table, int number)
    {
        double max;
        int index;
...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (country[i] == "р")
            {
                Rus[count_r] = new Russia(new string[] { answer_animal[i], answer_personality[i], answer_thing[i] }, new double[] { (100 * k1 / c1), (100 * k2 / c1), (100 * k3 / c1) });
                count_r++;
            }
            if (country[i] == "я")
            {
                Jap[count_j] = new Japan(new string[] { answer_animal[i], answer_personality[i], answer_thing[i] }, new double[] { (100 * k1 / c2), (100 * k2 / c2), (100 * k3 / c2) });
                count_j++;
            }
    ...
    private void Print(Country[] table, int count)
    {
        ...
        //var c = new Country();
        Country.Sort(statistic_animal, 0);
        Country.Sort(statistic_personality, 1);
        Country.Sort(statistic_personality, 2);
